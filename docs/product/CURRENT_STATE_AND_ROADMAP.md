...
- RM-31 osu! Private Proof Publish Policy / Public Projection Gate exists with a closed domain and local RPC gate for osu! provider/proof projection.
- RM-32 osu! Owner Proof Visibility Controls exists as owner-only private `/account` UI plus backend API mutation for private/public proof visibility preference.
- RM-33 osu! Public Projection Smoke / Projection QA exists as safe local allowlisted projection smoke for owner-public osu! provider/proof data.
- RM-34 osu! Public Profile Trust-Safety QA exists with neutral public copy, safe profile URL handling, minimum public renderer/mapper fix for allowlisted osu! DTOs, rollback documentation, and source guards.
- RM-35 osu! Production Readiness / Staging Go-No-Go exists with staging conditional-go, production no-go, environment checklist, callback checklist, staging smoke runbook, monitoring/logging minimums, rollback summary, and source guards.
...
- Automatic public osu! proof publication.
- Production osu! launch and owner go/no-go.
...
| RM-30 | osu! Owner Linking UI Hardening / Private Account UX | Done. Hardens private owner UX around linking status, backend link-intent, unlink confirmation, privacy copy, and account controls after full smoke completion. |
| RM-31 | osu! Private Proof Publish Policy / Public Projection Gate | Done. Defines and enforces a closed public projection gate for osu! proof data. |
| RM-32 | osu! Owner Proof Visibility Controls | Done. Adds explicit owner controls for proof visibility preference while public preference alone cannot bypass the projection gate. |
| RM-33 | osu! Public Projection Smoke / Projection QA | Done. Enables safe local public projection smoke only through allowlisted fields and full policy checks. |
| RM-34 | osu! Public Profile Trust-Safety QA | Done. Audits trust-safety, fixes the public renderer/mapper for osu! allowlisted DTOs, and keeps production blocked. |
| RM-35 | osu! Production Readiness / Staging Go-No-Go | This PR. Produces readiness docs, sets staging to `conditional-go`, and keeps production `no-go`. |
| RM-36 | osu! Staging Configuration / Manual Smoke | Next. Executes staging configuration and manual smoke after explicit owner approval. |
