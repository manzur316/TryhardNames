        ...
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="relative">
            <input
              ...
              className="w-full text-2xl md:text-4xl py-6 md:py-8 px-6 md:px-8 bg-background border-2 border-border/50 focus:border-primary focus:ring-0 text-foreground placeholder:text-foreground/20 rounded-xl transition-all duration-300 outline-none shadow-inner"
              maxLength={maxLength}
            />
            ...
            <AnimatePresence>
              {inputText.length > 0 && (
                <motion.button
                  ...
                  onClick={handleClear}
                  className="absolute right-6 top-1/2 -translate-y-1/2 p-2 bg-card hover:bg-border/50 rounded-full text-foreground/50 hover:text-foreground transition-colors"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {categorizedStyles[category].map((style) => (
                <div key={style.id}>
                  <FontStyleCard
                    label={style.name}
                    preview={convertText(inputText || 'Tryhard', style.id)}
                    isSelected={false}
                  />
                </div>
              ))}
        ...
        {!showMore && Object.keys(categorizedStyles).length > 3 && (
          ...
          >
            <button
              ...
              className="flex items-center gap-2 bg-card border border-border/50 hover:border-primary/50 text-foreground px-8 py-4 rounded-xl font-bold transition-all duration-300 hover:shadow-md group"
            >
              More styles
