...
import { gamerHubBreadcrumbJsonLd } from '@/seo/layoutBreadcrumbs.js';
...
export const GamerNamesLayout = ({
  ...
  const isHub = pathname === '/gamer-names';
  const childKey = pathname.startsWith('/gamer-names/') ? pathname.replace('/gamer-names/', '') : '';
  const jsonLd = [
    gamerHubBreadcrumbJsonLd(pathname),
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <nav className="flex items-center text-sm text-slate-500 dark:text-dark-400 mb-8 flex-wrap" aria-label="Breadcrumb">
          ...
          {!isHub ? (
            <>
              <Link to="/gamer-names" className="hover:text-purple-600 dark:hover:text-purple-400 transition-colors">Gamer Names</Link>
              <ChevronRight className="w-4 h-4 mx-2 opacity-50 flex-shrink-0" />
