...
import { AlertTriangle, CheckCircle2, ExternalLink, Link2, Loader2, LockKeyhole, RefreshCw, ShieldCheck, Unlink } from 'lucide-react';
import {
  createOsuLinkIntent,
...
function selectActiveConnection(connections) {
  return connections.find((connection) => connection.status === 'verified' && connection.visibility === 'private')
    || connections.find((connection) => connection.status === 'stale')
    || null;
}

function buildStatusModel({ runtime, latestConnection, activeConnection, isRuntimeConfigured, isConnected, isPassportLoading }) {
  ...
  if (isConnected) {
    return {
      label: 'Connected private',
      detail: 'This Passport has a verified private osu! ownership proof.',
